...
public final class RatPoly {
    ...
    public RatPoly negate() {
    	...
    	if(this.degree==0&&this.coeffs.length==0) {
        	return RatPoly.ZERO;
        }
        RatNum[] new_coeffs = new RatNum[this.degree+1];
        for(int i=0;i<new_coeffs.length;i++) {
        	new_coeffs[i] = this.coeffs[i].negate();
        }
    ...
    public RatPoly add(RatPoly p) {
        ...
        RatNum[] new_coeffs = new RatNum[e];
        
        for(int i =0;i<e;i++) {
        	new_coeffs[i]= this.getCoeff(i).add(p.getCoeff(i));
        }
        int new_degree = e-1;
        while(new_coeffs[new_degree].compareTo(RatNum.ZERO)==0&&new_degree>0) {
        	new_degree--;
        }
        RatNum[] post_coeffs = new RatNum[new_degree+1];
        for(int i=0;i<=new_degree;i++) {
        	post_coeffs[i] = new_coeffs[i];
        }
        if(new_degree==0&&new_coeffs[0].compareTo(RatNum.ZERO)==0) {
        	return RatPoly.ZERO;
        }else {
        	return new RatPoly(post_coeffs);
        }
    }
    ...
    public RatPoly sub(RatPoly p) {
        ...
        RatNum[] new_coeffs = new RatNum[e];
        for(int i =0;i<e;i++) {
        	new_coeffs[i]= this.getCoeff(i).sub(p.getCoeff(i));
        }
        int new_degree = e-1;
        while(new_coeffs[new_degree].compareTo(RatNum.ZERO)==0&&new_degree>0) {
        	new_degree--;
    ...
    public RatPoly mul(RatPoly p) {
        ...
        if(this.isNaN()||p.isNaN()) {
        	...
        	int e = this.degree+p.degree+1;
        	RatNum[] new_coeffs = new RatNum[e];
        	for(int i =0;i<e;i++) {
        		int j =0, k= i-j;
        		RatNum a = new RatNum(0);
        		while(k>=0) {
        			a=a.add(this.getCoeff(j).mul(p.getCoeff(k)));
        			j++;
        			k--;
        		}
        		new_coeffs[i] = a;
    ...
    public double eval(double d) {
        ...
        if(this.isNaN()) {
        	...
        	for(int i=this.degree;i>=0;i--) {
        		result = result*d+this.getCoeff(i).doubleValue();
        	}
        	return result;
        }
    }  
    ...
    public RatPoly differentiate() {
        ...
        if(this.isNaN()) {
        	...
        	RatNum[] new_coeffs = new RatNum[this.degree];
        	for(int i=1;i<=this.degree;i++) {
        		new_coeffs[i-1] = this.getCoeff(i).mul(new RatNum(i));
        	}
        	return new RatPoly(new_coeffs);
        }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
    	...
    	if(this.isNaN()) {
        	return RatPoly.NaN;
        }else if(this.degree==0&&this.coeffs.length ==0){
        	RatNum[] constant = new RatNum[1];
        	constant[0]=integrationConstant;
        	if(integrationConstant.compareTo(RatNum.ZERO)==0) {
        		return new RatPoly();
        	}
        	return new RatPoly(constant);
        }else {
        	RatNum[] new_coeffs = new RatNum[this.degree+2];
        	for(int i=0;i<=this.degree;i++) {
        		new_coeffs[i+1] = this.getCoeff(i).div(new RatNum(i+1));
        	}
    ...
    public double integrate(double lowerBound, double upperBound) {
        ...
        RatPoly x = this.antiDifferentiate(RatNum.ZERO);
        return x.eval(upperBound)-x.eval(lowerBound);
    }
    ...
    public RatPoly div(RatPoly p) {
    	if(this.isNaN()||p.isNaN()) {
    		return RatPoly.NaN;
    	}else if(p.degree==0&&p.coeffs.length==0) {
    		return RatPoly.NaN;
    	}else if(this.degree==0&&this.coeffs.length==0) {
    		return RatPoly.ZERO;
    	}
    	...
    	RatNum [] r = new RatNum[this.degree+1];
    	for(int i=0;i<=this.degree;i++) {
    		r[i] = this.coeffs[i];
    	}
    	
    	RatNum[] q = new RatNum[u_d-p_d+1];
    	int i = u_d-p_d;
    
    	while(i>=0) {
    		int j=p_d;
    		q[i] = r[i+j].div(p.getCoeff(j));
    		while(j>=0) {
    			r[i+j] = r[i+j].sub(q[i].mul(p.getCoeff(j)));
    			j--;
    		}
    		i--;
    	}
